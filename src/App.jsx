...
const RELEASES=[
  {ver:"v1.4", date:"2026-06-21",
    en:["UC-05/11/12/13/14 pages — full BRD use-case coverage","Data-flow funnel on Data Readiness (6 sources → BIDSC)","Internal UC codes removed from the UI","Login title fixed to MoMAH"],
    zh:["新增 UC-05/11/12/13/14 五页 —— BRD 用例全覆盖","Data Readiness 数据流向漏斗(6 源 → BIDSC)","移除界面中的 UC 编号","登录标题修正为 MoMAH"]},
  {ver:"v1.3", date:"2026-06-20",
...
function ReleaseNotes({onClose}){
  ...
  return (<Modal title={t("rel_title")} onClose={onClose}>
    <div className="rel-time">
      {RELEASES.map((r,i)=>(<div key={r.ver} className={"rel-item"+(i===0?" cur":"")}>
        <span className="rel-dot"/>
        <div className="rel-head"><b>{r.ver}</b> <span className="muted" style={{fontSize:12}}>· {r.date}</span>{i===0?<span className="chip" style={{marginInlineStart:8}}>{t("rel_current")}</span>:null}</div>
  ...
  return (<div className="sidebar">
    ...
    <div className="side-foot">
      <button className="ver-chip" onClick={()=>setRel(true)} title={t("rel_title")}>
        <span className="ver-dot"/> {APP_VER} <span className="muted" style={{fontSize:11,fontWeight:400}}>· {RELEASES[0].date}</span>
      </button>
    </div>
...
Object.assign(I18N.en,{ applyReco:"Apply AI suggestion", save_over:"⚠ Exceeds 43%", rel_title:"Release notes", rel_current:"Current" });
Object.assign(I18N.zh,{ applyReco:"应用 AI 建议", save_over:"⚠ 超出 43%", rel_title:"更新日志", rel_current:"当前" });
Object.assign(I18N.ar,{ applyReco:"تطبيق توصية الذكاء", save_over:"⚠ يتجاوز ٤٣٪", rel_title:"سجل الإصدارات", rel_current:"الحالي" });
